...
public func fetch(_ request: FetchRequest) async throws -> any FetchResponse {
    return try await Fetcher.fetch(request)
}

public func fetch(_ url: URL, _ options: FetchRequest.Options = .options()) async throws -> any FetchResponse {
    let request = FetchRequest(url, options)
    return try await fetch(request)
}

public func fetch(_ urlPath: String, _ options: FetchRequest.Options = .options()) async throws -> any FetchResponse {
...
) async throws -> any FetchResponse {
    guard
